...
bool ChatHandler::HandleGonameCommand(char* args)
{
    ...
    if (target)
    {
        ...
        if (target->GetTransport())
            _player->m_movementInfo.SetTransportData(target->m_movementInfo.t_guid, target->m_movementInfo.t_pos.x, target->m_movementInfo.t_pos.y, target->m_movementInfo.t_pos.z, target->m_movementInfo.t_pos.o, target->m_movementInfo.t_time, -1);
