...
uint32 WaypointMovementGenerator<Creature>::BuildIntPath(PointsArray& path, Creature& creature, Vector3 const& endPos) const
{
    ...
    static const float  TerrainStep     = 2.5f;             // min step for each int point in yard
    ...
    if (distance >= MinimumDistance)
    {
        ...
        while (true)
        {
            ...
            currGroundZ = map->GetHeight(creature.GetPhaseMask(), currPos.x, currPos.y, currPos.z + 1.0f) + 0.2f;
