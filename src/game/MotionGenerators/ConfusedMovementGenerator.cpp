...
void ConfusedMovementGenerator<T>::Initialize(T& unit)
{
    ...
    {
        if (unit.IsNonMeleeSpellCasted(false))
            unit.CastStop();

        if (unit.getVictim() && !unit.CanReachWithMeleeAttack(unit.getVictim()))
            unit.SendMeleeAttackStop(unit.getVictim());
    }
...
bool ConfusedMovementGenerator<T>::Update(T& unit, const uint32& diff)
{
    ...
    else
    {
        ...
        if (i_nextMoveTime.Passed())
        {
            ...
            if (unit.GetMap()->GetReachableRandomPosition(&unit, destX, destY, destZ, 10.0f) && unit.IsWithinLOSForMe(destX, destY, destZ, unit.GetCollisionHeight()))
            {
