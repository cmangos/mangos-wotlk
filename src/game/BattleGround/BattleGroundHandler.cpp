...
void WorldSession::HandleBattlemasterJoinOpcode(WorldPacket& recv_data)
{
    ...
    if (joinAsGroup)
    {
        ...
        for (GroupReference* itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
        {
            ...
            sBattleGroundMgr.BuildBattleGroundStatusPacket(&data, bg, queueSlot, STATUS_WAIT_QUEUE, avgTime, 0, ginfo->arenaType, member->GetBGTeam());
            member->GetSession()->SendPacket(&data);
            sBattleGroundMgr.BuildGroupJoinedBattlegroundPacket(&data, err);
            member->GetSession()->SendPacket(&data);
    ...
    else
    {
        ...
        WorldPacket data;
        // send status packet (in queue)
        sBattleGroundMgr.BuildBattleGroundStatusPacket(&data, bg, queueSlot, STATUS_WAIT_QUEUE, avgTime, 0, ginfo->arenaType, _player->GetBGTeam());
        SendPacket(&data);
...
void WorldSession::HandleBattleFieldPortOpcode(WorldPacket& recv_data)
{
    ...
    switch (action)
    {
        ...
        case 0:                                         // leave queue
            ...
            sBattleGroundMgr.BuildBattleGroundStatusPacket(&data, bg, queueSlot, STATUS_NONE, 0, 0, ARENA_TYPE_NONE, _player->GetBGTeam());
...
void WorldSession::HandleBattlefieldStatusOpcode(WorldPacket& /*recv_data*/)
{
    ...
    for (uint8 i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i)
    {
        ...
        if (ginfo.IsInvitedToBGInstanceGUID)
        {
            ...
            sBattleGroundMgr.BuildBattleGroundStatusPacket(&data, bg, i, STATUS_WAIT_JOIN, remainingTime, 0, arenaType, _player->GetBGTeam());
            SendPacket(&data);
        }
        else
        {
            ...
            sBattleGroundMgr.BuildBattleGroundStatusPacket(&data, bg, i, STATUS_WAIT_QUEUE, avgTime, WorldTimer::getMSTimeDiff(ginfo.JoinTime, WorldTimer::getMSTime()), arenaType, _player->GetBGTeam());
            SendPacket(&data);
        }
    }
}
...
void WorldSession::HandleBattlemasterJoinArena(WorldPacket& recv_data)
{
    ...
    if (asGroup)
    {
        ...
        for (GroupReference* itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
        {
            ...
            // send status packet (in queue)
            sBattleGroundMgr.BuildBattleGroundStatusPacket(&data, bg, queueSlot, STATUS_WAIT_QUEUE, avgTime, 0, arenatype, member->GetBGTeam());
            member->GetSession()->SendPacket(&data);
            sBattleGroundMgr.BuildGroupJoinedBattlegroundPacket(&data, err);
            member->GetSession()->SendPacket(&data);
    ...
    else
    {
        ...
        WorldPacket data;
        // send status packet (in queue)
        sBattleGroundMgr.BuildBattleGroundStatusPacket(&data, bg, queueSlot, STATUS_WAIT_QUEUE, avgTime, 0, arenatype, _player->GetBGTeam());
        SendPacket(&data);
