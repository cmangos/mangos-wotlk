    ...
    do
    {
        ...
        switch (tmp.command)
        {
            ...
            case SCRIPT_COMMAND_UPDATE_TEMPLATE:            // 44
            {
                ...
                {
                    sLog.outErrorDb("Table `%s` uses nonexistent creature entry %u in SCRIPT_COMMAND_PLAYER_LEARN_SPELL for script id %u.", tablename, tmp.updateTemplate.newTemplate, tmp.id);
                    continue;
                }
                if (tmp.updateTemplate.newFactionTeam != 0 && tmp.updateTemplate.newFactionTeam != 1)
                {
                    sLog.outErrorDb("Table `%s` uses nonexistent faction team %u in SCRIPT_COMMAND_PLAYER_LEARN_SPELL for script id %u.", tablename, tmp.updateTemplate.newFactionTeam, tmp.id);
                    continue;
                }
                break;
            }
