...
void WardenMac::HandleHashResult(ByteBuffer &buff)
{
    ...
    DEBUG_LOG("WARDEN: Request hash reply: succeed");
...
void WardenMac::HandleData(ByteBuffer &buff)
{
    ...
    if (Client->GetPlayer())
        Client->GetPlayer()->GetAntiCheat()->DoAntiCheatCheck(CHECK_WARDEN_MEMORY,FAKE_CHECK,found);
