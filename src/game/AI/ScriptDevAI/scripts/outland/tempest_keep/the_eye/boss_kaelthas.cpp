...
struct boss_kaelthasAI : public ScriptedAI
{
    ...
    void ExecuteActions()
    {
        ...
        if (!m_unit->CanReactInCombat() || m_unit->hasUnitState(UNIT_STAT_PROPELLED | UNIT_STAT_SEEKING_ASSISTANCE | UNIT_STAT_CHANNELING) || m_unit->IsNonMeleeSpellCasted(false))
            return;
